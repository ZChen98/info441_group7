...
async function loadAllDorm() {
  ...
  try {
    ...
    if (dormJson.status == "error") {
      document.getElementById("dorm_box").innerText =
        "There was an error: " + dormJson.error;
    } else {
      renderDorm(dormJson);
    }
  } catch (error) {
  ...
  if (dormName !== null) {
    try {
      ...
      if (responseJSON.status == "error") {
        ...
        document.getElementById("dorm_box").innerHTML =
          "No result found. Try a different building name!";
      } else {
